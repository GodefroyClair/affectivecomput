...
respi_add_variables <- function(df) {
  ...
  df <- df %>% mutate( respi_clean = replace(respi_clean, nom.experience %in% c("LM","DA2","DA3"), NA))
  expect_true(any(is.na( df %>% filter(nom.experience %in% c("LM","DA2","DA3")) %>% select(respi_clean))))
  expect_false(any(is.na( df %>% filter(!nom.experience %in% c("LM","DA2","DA3")) %>% select(respi_clean))))
  
  #### NETTOYER ###
  #AW, FS1, DA, AB, CLP
  
  #AW
  df <- df %>% mutate( respi_clean = 
                         replace(respi_clean, nom.experience == "AW" & respiration >= 0,
                                 NA))
  ...
  date_seuil <- df %>% 
    filter(nom.experience == "AB" & respiration <= -18.66 & lubridate::minute(date) >= 43) %>%
    select(date) %>% summarise(min(date)) %>% `[[`(.,1)
  ...
  #CLP
  min_date <- df %>% filter(nom.experience == "CLP" & respiration < -18.65) %>% select(date) %>% summarise(min(date)) %>% `[[`(.,1)
  df <- df %>% mutate( respi_clean = 
                         replace(respi_clean, nom.experience == "CLP" & date >= min_date ,
                                 NA))
  ...
  #========== SEPARATE LOW FREQUENCIES (trend) ==============#
  df <- df %>% group_by(nom.experience) %>% mutate(respi_clean_hl = clean_low_freq(respi_clean_h, .01))
  
  df <- df %>% group_by(nom.experience) %>% mutate(respi_trend = clean_high_freq(respi_clean_h, .01))
  ...
  if (DEBUG) {
    ...
    df_test %>% group_by(z) %>% mutate(m_loess = maybeLoess()) 
    #magritr
  }
